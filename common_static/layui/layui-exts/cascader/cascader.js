    ...
    Cascader.prototype={
        ...
        initOption: function () {
            ...
            self.option.elem?(function(){
                ...
                self.elemValue=self.option.elemValue;
            })():(function() {
                throw "缺少elem节点选择器";
            })();
            ...
            var string =  laytpl(
                '<ul class="urp-cascader-child">'+
                    '{{# for(var i=0;i<d.length;i++){ }}'+
                        '<li>{{ d[i].categoryName }}<i class="layui-icon layui-icon-right" ></i></li>'+
                    '{{# } }}'+
                '</ul>'
            ).render(this.d);
        ...
        initChild: function (triggerData,node) {
            ...
            this.textArr.push(blockData.categoryName);
            this.valueArr.length=this.floor;
            this.valueArr.push(blockData.categoryId);
            var string =  laytpl(
                '<ul class="urp-cascader-child">'+
                    '{{# for(var i=0;i< d.length;i++){ }}'+
                        '<li>{{ d[i].categoryName }}<i class="layui-icon layui-icon-right"></i></li>'+
            ...
            if(this.changeOnSelect){
                ...
                //console.log(2);
                $(this.elemValue).val(this.valueArr[this.valueArr.length-1]);
        ...
        finishInitData: function (triggerData) {
            ...
            this.textArr.length=this.floor;
            this.textArr.push(this.blockData.categoryName);
            this.valueArr.length=this.floor;
            this.valueArr.push(this.blockData.categoryId);
            // 文本拼接
